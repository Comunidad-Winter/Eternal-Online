...
On Error Resume Next
    ...
    Select Case incomingData.PeekByte()
        ...
        Case ServerPacketID.FailAttacksInHead
            Call HandleFailAttacksInHead
            
        Case ServerPacketID.AttacksInHead
            Call HandleAttacksInHead
        ...
        Case ServerPacketID.CreateParticle          ' CCP
        ...
        Case ServerPacketID.CharVeneno             'CV
            Call HandleCharVeneno
...
'***************************************************
    ...
    Call RemoveDialog(incomingData.ReadInteger())
End Sub
...
'***************************************************
    ...
    'Close connection
    If frmMain.Client.State <> (sckClosed Or sckConnecting) Then _
...
'***************************************************
    ...
    Call InvOroComUsu(0).Initialize(frmComerciarUsu.picInvOroProp, INV_GOLD_SLOTS, , Tile_Pixel_Size * 2, Tile_Pixel_Size, Tile_Pixel_Size / 2)
    Call InvOroComUsu(1).Initialize(frmComerciarUsu.picInvOroOfertaProp, INV_GOLD_SLOTS, , Tile_Pixel_Size * 2, Tile_Pixel_Size, Tile_Pixel_Size / 2)
    Call InvOroComUsu(2).Initialize(frmComerciarUsu.picInvOroOfertaOtro, INV_GOLD_SLOTS, , Tile_Pixel_Size * 2, Tile_Pixel_Size, Tile_Pixel_Size / 2)
...
Private Sub HandleFailAttacksInHead()
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'
'***************************************************
    ...
    Dim char_index As Integer '// CharIndex
    
    'Remove packet ID
    Call incomingData.ReadByte
    char_index = incomingData.ReadInteger()
    
    If char_index = UserCharIndex Then '// SI SOY YO QUE DIGA "FALLAS"
        engine.Char_Dialog_Set char_index, "*Fallas*", D3DColorXRGB(255, 0, 0), 5, 2
    Else '// Si no soy yo que diga "Falla"
        engine.Char_Dialog_Set char_index, "*Falla*", D3DColorXRGB(255, 0, 0), 5, 2
    End If
End Sub
Private Sub HandleAttacksInHead()
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'
'***************************************************
    ...
    Dim Attack As Long '// Daño provocado
    Dim char_index As Integer '// CharIndex
    
    'Remove packet ID
    Call incomingData.ReadByte
    Attack = incomingData.ReadLong()
    char_index = incomingData.ReadInteger()
    
    If Attack >= 150 Then
        engine.Char_Dialog_Set char_index, "¡" & Attack & "!", D3DColorXRGB(255, 0, 0), 5, 2
    Else
        engine.Char_Dialog_Set char_index, Attack, D3DColorXRGB(255, 0, 0), 5, 2
    End If
...
On Error GoTo errhandler
    ...
    If Len(charlist(CharIndex).Nombre) <= 1 Then
        RemoveDialogsNPCArea
    End If
    
    'Only add the chat if the character exists (a CharacterRemove may have been sent to the PC / NPC area before the buffer was flushed)
    If charlist(CharIndex).active Then _
        engine.Char_Dialog_Set CharIndex, chat, D3DColorXRGB(r, g, b), 10
...
On Error GoTo errhandler
    ...
    With charlist(CharIndex)
        ...
        '// Lo arme especificamente para los NPCS y por el sistema de montura.
        .Scroll_Pixels_Per_Frame = Speed
...
'***************************************************
    ...
    CharIndex = incomingData.ReadInteger()
    Effect = incomingData.ReadBoolean()

    If Effect Then
        charlist(CharIndex).StatusAlpha = Effect
    Else
...
'***************************************************
    ...
    MapData(X, Y).Blocked = incomingData.ReadByte()
End Sub
...
'***************************************************
    ...
    If bRain Then
        ...
        End If
    Else
        If frmMain.tTrueno.Enabled = True Then frmMain.tTrueno.Enabled = False
        If MapInfo.Terrain = 0 Or MapInfo.Terrain = 3 Then
            Call Audio.StopWave(SoundRainIndex)
            SoundRainIndex = 0
            Call Audio.PlayWave(SND_LLUVIAEND)
        End If
    End If
End Sub
...
Private Sub HandleCharVeneno()
    ...
    CharIndex = incomingData.ReadInteger()
    charlist(CharIndex).Envenenado = incomingData.ReadByte()
End Sub
