...
On Error Resume Next
    ...
    Select Case packetID
        ...
        Case ClientPacketID.RegresarHogar
            Call HandleRegresarHogar(UserIndex)
...
On Error GoTo Errhandler
    ...
    If seguridad_clones_validar(UserList(UserIndex).ip) = False Then
        Call WriteErrorMsg(UserIndex, "Estás intentando crear demasiados personajes.")
        Call FlushBuffer(UserIndex)
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
        ...
        If LenB(Chat) <> 0 Then
            ...
            If Not (.flags.AdminInvisible = 1) Then
                ...
                Else
                    ...
                    Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageConsoleMsg(.name & "> " & Chat, FontTypeNames.FONTTYPE_TALK))
                End If
            Else
                If RTrim(Chat) <> "" Then
...
On Error GoTo Errhandler
    With UserList(UserIndex)
        ...
        If modGuilds.CrearNuevoClan(UserIndex, desc, GuildName, site, codex, .FundandoGuildAlineacion, errorStr) Then
            Call SendData(SendTarget.ToAll, UserIndex, PrepareMessageConsoleMsg(.name & " fundó el clan " & GuildName & " de alineación " & modGuilds.GuildAlignment(.GuildIndex) & " y ahora podra acceder a los torneos de Matchmaking.", FontTypeNames.FONTTYPE_GUILD))
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If ((Slot < 0 Or Slot > MAX_INVENTORY_SLOTS) And Slot <> FLAGORO) Then Exit Sub
    ...
    With UserList(UserIndex)
        ...
        If .flags.Meditando Then
            ...
            If .Stats.ELV < 15 Then
                .Char.Particle = ParticleIDs.M_NEWBIE
            ElseIf .Stats.ELV < 25 Then
                .Char.Particle = ParticleIDs.M_VERSION_TWO
            ElseIf .Stats.ELV < 35 Then
                .Char.Particle = ParticleIDs.M_VERSION_THREE
            ElseIf .Stats.ELV < 50 Then
                .Char.Particle = ParticleIDs.M_VERSION_FOUR
            End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If Not UserList(UserIndex).Stats.ELV = 50 Then '¿Soy nivel maximo?
            Call WriteConsoleMsg(UserIndex, "¡No eres nivel maximo!", FontTypeNames.FONTTYPE_INFOBOLD)
            Exit Sub
        End If
        
        'If Not UserList(UserIndex) Then '¿Tengo 100 de liderazgo?
        '    Call WriteConsoleMsg(UserIndex, "¡No eres nivel maximo!", FontTypeNames.FONTTYPE_INFOBOLD)
        '    Exit Sub
        'End If
...
'***************************************************
    With UserList(UserIndex)
        ...
        Call GrabarMapa(.Pos.Map, App.Path & "\Resources\WorldBackUp\Mapa" & .Pos.Map)
...
Public Sub WriteCharacterCreate(ByVal UserIndex As Integer, ByVal Speed As Byte, ByVal body As Integer, ByVal Head As Integer, ByVal heading As eHeading, _
...
'***************************************************
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageCharacterCreate(Speed, body, Head, heading, CharIndex, X, Y, weapon, shield, FX, FXLoops, _
...
Public Sub WriteCharacterRemove(ByVal UserIndex As Integer, ByVal CharIndex As Integer, ByVal StatusAlpha As Boolean)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "CharacterRemove" message to the given user's outgoing data buffer
'***************************************************
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageCharacterRemove(CharIndex, StatusAlpha))
Exit Sub
...
Public Function PrepareMessageCharacterRemove(ByVal CharIndex As Integer, ByVal StatusAlpha As Boolean) As String
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Prepares the "CharacterRemove" message and returns it
...
Public Function PrepareMessageCharacterCreate(ByVal Speed As Byte, ByVal body As Integer, ByVal Head As Integer, ByVal heading As eHeading, _
                                ByVal CharIndex As Integer, ByVal X As Byte, ByVal Y As Byte, ByVal weapon As Integer, ByVal shield As Integer, _
                                ByVal FX As Integer, ByVal FXLoops As Integer, ByVal helmet As Integer, ByVal name As String, ByVal NickColor As Byte, _
...
Public Function PrepareMessageFailAttacksInHead(ByVal Char_Index As Integer) As String
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.FailAttacksInHead)
        Call .WriteInteger(Char_Index)
        PrepareMessageFailAttacksInHead = .ReadASCIIStringFixed(.length)
    End With
End Function

Public Function PrepareMessageAttacksInHead(ByVal Daño As Long, ByVal Char_Index As Integer) As String
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.AttacksInHead)
        Call .WriteLong(Daño)
        Call .WriteInteger(Char_Index)
        PrepareMessageAttacksInHead = .ReadASCIIStringFixed(.length)
    End With
End Function

Public Sub WriteAttacksInHead(ByVal UserIndex As Integer, ByVal Daño As Long, ByVal Char_Index As Integer)
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageAttacksInHead(Daño, Char_Index))
Exit Sub
...
Public Sub WriteFailAttacksInHead(ByVal UserIndex As Integer, ByVal Char_Index As Integer)
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageFailAttacksInHead(Char_Index))
Exit Sub
...
Public Sub WriteCharVeneno(ByVal UserIndex As Integer, ByVal CharIndex As Integer, ByVal uEnvenenado As Byte)
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageCharVeneno(CharIndex, uEnvenenado))
Exit Sub
...
Errhandler:
    ...
    End If
End Sub
Public Function PrepareMessageCharVeneno(ByVal CharIndex As Integer, ByVal uEnvenenado As Byte) As String
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Prepares the "SetInvisible" message and returns it.
'***************************************************
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.CharVeneno)
        
        Call .WriteInteger(CharIndex)
        Call .WriteBoolean(uEnvenenado)
        
        PrepareMessageCharVeneno = .ReadASCIIStringFixed(.length)
    End With
End Function
Public Sub HandleRegresarHogar(ByVal UserIndex As Integer)
'=================================================================
' Autor: ZenitraM
' Last Modification: 17/07/2020
' Comments: Runa desde HUD y no desde OBJ
'=================================================================
Dim UserHogar As WorldPos
    ...
    With UserList(UserIndex)
        ...
        Select Case .Hogar
            Case eCiudad.cOnirem
                UserHogar = Onirem
                
            Case eCiudad.cOsurac
                UserHogar = Osurac
        End Select

        If UserList(UserIndex).flags.Muerto = 0 Then
            If MapInfo(UserList(UserIndex).Pos.Map).Pk = True Then
                Call WriteConsoleMsg(UserIndex, "No puedes usar la runa de teletransportación en zona inseguras.", FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
    
            If UserList(UserIndex).Counters.Pena > 0 Then
                Call WriteConsoleMsg(UserIndex, "No puedes usar la runa de teletransportación en la carcel!.", FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
        
            'Teletransportamos el personaje a su hogar
