            ...
            If .TileExit.Map > 0 And .TileExit.Map <= NumMaps Then
                ...
                ElseIf UCase$(MapInfo(DestPos.Map).Restringir) = "FACCION" Then '¿Es mapa de faccionarios?
                    ...
                    Else 'No es Faccionario
                        ...
                        End If
                    End If
                ElseIf UserList(UserIndex).Stats.ELV > MapInfo(DestPos.Map).LevelMAX Or UserList(UserIndex).Stats.ELV < MapInfo(DestPos.Map).LevelMIN Then
                        Call WriteConsoleMsg(UserIndex, "Para entrar al mapa tienes que ser como minimo nivel " & MapInfo(DestPos.Map).LevelMIN & " y " & MapInfo(DestPos.Map).LevelMAX & ".", FontTypeNames.FONTTYPE_INFO)
                        Call ClosestStablePos(UserList(UserIndex).Pos, nPos)
    ...
    If InMapBounds(Map, X, Y) Then
           ...
           If UserList(TempCharIndex).flags.AdminInvisible = 0 Or .flags.Privilegios And PlayerType.Dios Then
                With UserList(TempCharIndex)
                    If LenB(.DescRM) = 0 And .showName Then 'No tiene descRM y quiere que se vea su nombre.
                        If Len(.desc) > 0 Then
                            Stat = "Ves a " & .name & Stat & " - " & .desc & " (" & ListaClases(.clase) & " " & ListaRazas(.raza) & " Nivel " & .Stats.ELV & " | "
                        Else
                            Stat = "Ves a " & UserList(TempCharIndex).name & Stat & " (" & ListaClases(.clase) & " " & ListaRazas(.raza) & " Nivel " & .Stats.ELV & " | "
                        End If
                        '& " <Ciudadano de " & UserList(UserIndex).Hogar & ">
                        
                        If .flags.Comerciando Then
                            Stat = Stat & "Comerciando)"
                        ElseIf .flags.Envenenado Then
                            Stat = Stat & "Envenenado)"
                        ElseIf .flags.Inmovilizado Then
                            Stat = Stat & "Inmovilizado)"
                        ElseIf .flags.Paralizado Then
                            Stat = Stat & "Paralizado)"
                        ElseIf .flags.Estupidez Then
                            Stat = Stat & "Estupidizado)"
                        ElseIf .flags.Meditando Then
                            Stat = Stat & "Meditando)"
                        ElseIf .flags.invisible Then
                            Stat = Stat & "Invisible)"
                        ElseIf .flags.Oculto Then
                            Stat = Stat & "Oculto)"
                        Else
                        
                        If .Stats.MinHp < (.Stats.MaxHp * 0.05) Then
                            Stat = Stat & "Muerto)"
                        ElseIf .Stats.MinHp < (.Stats.MaxHp * 0.1) Then
                            Stat = Stat & "Casi muerto)"
                        ElseIf .Stats.MinHp < (.Stats.MaxHp * 0.25) Then
                            Stat = Stat & "Muy Malherido)"
                        ElseIf .Stats.MinHp < (.Stats.MaxHp * 0.5) Then
                            Stat = Stat & "Malherido)"
                        ElseIf .Stats.MinHp < (.Stats.MaxHp * 0.75) Then
                            Stat = Stat & "Herido)"
                        ...
                        Else
                            If Not .flags.Privilegios And PlayerType.User Then
                                Stat = Stat & " <Eternal Online Staff>"
        ...
        With .flags
            If FoundChar = 2 Then '¿Encontro un NPC?
                ...
                Else
                    If .Muerto = 0 Then
                        If SupervivenciaSkill >= 0 And SupervivenciaSkill < 60 Then
