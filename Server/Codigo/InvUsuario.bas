...
With UserList(UserIndex)
    ...
    If (Cantidad > 0) And (Cantidad <= .Stats.GLD) Then
            ...
            Do While (Cantidad > 0)
                ...
                If MiObj.Amount >= 10000 Then
                    MiObj.ObjIndex = iORO
                Else
                    If MiObj.Amount >= 1 And Not MiObj.Amount > 1000 And Not MiObj.Amount > 10000 Then
                        MiObj.ObjIndex = iOROLow
                    Else
                        MiObj.ObjIndex = iOROMid
                    End If
                End If
...
Sub DropObj(ByVal UserIndex As Integer, ByVal Slot As Byte, ByVal num As Long, ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer)
'***************************************************
'Author: Unknown
'Last Modification: -
'
            ...
            Case eOBJType.otArmadura
                ...
                If ClasePuedeUsarItem(UserIndex, ObjIndex, sMotivo) And _
                   ...
                   'Si esta equipado lo quita
                    ...
                    Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_ARMADURAS, .Pos.X, .Pos.Y))
    ...
    With UserList(UserIndex)
        ...
        Select Case Obj.OBJType
            Case eOBJType.otUseOnce
                ...
                If .Stats.MinAGU > 5 Then
                    .Stats.MinAGU = .Stats.MinAGU - RandomNumber(1, 4)
                End If
                ...
                Select Case .flags.TipoPocion
                    ...
                    Case 1 'Modif la agilidad
                        ...
                        If .flags.AdminInvisible = 1 Then
                            Call EnviarDatosASlot(UserIndex, PrepareMessagePlayWave(SND_POCION, .Pos.X, .Pos.Y))
                        Else
                            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_POCION, .Pos.X, .Pos.Y))
                        End If
                    ...
                    Case 5 ' Pocion violeta
                        If .flags.Envenenado = 1 Then
                            ...
                            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(.Char.CharIndex, FXIDs.FXCURARVENENO, 0))
                            ...
                            End If
                        Else
                            Call WriteConsoleMsg(UserIndex, "No estas envenenado.", FontTypeNames.FONTTYPE_INFO)
                        End If
                        
                    Case 6  ' Pocion Remover Paralisis
                        If .flags.Paralizado = 1 Then
                            .flags.Paralizado = 0
                            Call WriteParalizeOK(UserIndex)
                            Call WriteConsoleMsg(UserIndex, "Te has removido el paralisis!", FontTypeNames.FONTTYPE_INFO)
                            ...
                            End If
                        Else
                            Call WriteConsoleMsg(UserIndex, "No estas paralizado.", FontTypeNames.FONTTYPE_INFO)
                        End If
                        
                    Case 7  ' Pocion de Invisibilidad
                        If .flags.invisible = 0 Then
                            .flags.invisible = 1
                            Call SetInvisible(UserIndex, UserList(UserIndex).Char.CharIndex, True)
                            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(.Char.CharIndex, FXIDs.FXINVSIBILE, 0))
                            Call WriteConsoleMsg(UserIndex, "Te has hecho invisble.", FontTypeNames.FONTTYPE_INFO)
                            ...
                            End If
                        Else
                            Call WriteConsoleMsg(UserIndex, "Ya te encuentras invisible.", FontTypeNames.FONTTYPE_INFO)
                        End If
               End Select
               Call WriteUpdateUserStats(UserIndex)
               Call UpdateUserInv(False, UserIndex, Slot)
