...
'***************************************************
    ...
    With Npclist(NpcIndex)
        ...
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageAttacksInHead(Daño, UserList(UserIndex).Char.CharIndex))
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Call SendData(SendTarget.ToNPCArea, NpcIndex, PrepareMessageAttacksInHead(Daño, Npclist(NpcIndex).Char.CharIndex))
...
'*************************************************
    ...
    Else
        ...
        Call SendData(SendTarget.ToNPCArea, NpcIndex, PrepareMessageFailAttacksInHead(Npclist(NpcIndex).Char.CharIndex))
...
On Error GoTo Errhandler
    ...
    Else
        ...
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageFailAttacksInHead(UserList(UserIndex).Char.CharIndex))
    ...
    With UserList(UserIndex)
        ...
        Else
            If .Genero = eGenero.Hombre Then
                Call WriteConsoleMsg(UserIndex, "Estás muy cansado.", FontTypeNames.FONTTYPE_INFO)
            Else
                Call WriteConsoleMsg(UserIndex, "Estás muy cansada.", FontTypeNames.FONTTYPE_INFO)
            End If
            Exit Sub
        End If
        ...
        index = MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).ObjInfo.ObjIndex 'encontramos objeto
        
        If index > 0 Then 'usamos el if para que se centre en ese solo obj
            If .flags.Muerto = 1 Or .flags.Descansar Or .flags.Meditando Then Exit Sub 'Si estamos muertos, meditando o descansando no podemos trabajar.
        
            'Talar
            If ObjData(index).OBJType = otArboles Then '¿es un arbol?
                If Not .Invent.WeaponEqpObjIndex = HACHA_LEÑADOR Then '¿Tengo un hacha?
                    Call WriteConsoleMsg(UserIndex, "Necesitas un hacha de leñador para poder talar.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
                
                If MapInfo(.Pos.Map).Pk = False Then '¿estamos en zona segura?
            ...
            'Minar
            If ObjData(index).OBJType = otYacimiento Then  '¿Es un yacimiento?
                If Not .Invent.WeaponEqpObjIndex = PIQUETE_MINERO Then '¿Tengo un pico?
                    Call WriteConsoleMsg(UserIndex, "Necesitas un pico para poder minar.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
...
On Error GoTo Errhandler
    ...
    With UserList(AtacanteIndex)
        ...
        Else
            ...
            Call SendData(SendTarget.ToPCArea, AtacanteIndex, PrepareMessageFailAttacksInHead(UserList(AtacanteIndex).Char.CharIndex))
            Call SendData(SendTarget.ToPCArea, VictimaIndex, PrepareMessageFailAttacksInHead(UserList(AtacanteIndex).Char.CharIndex))
...
On Error GoTo Errhandler
    ...
    With UserList(AtacanteIndex)
        ...
        Call SendData(SendTarget.ToPCArea, AtacanteIndex, PrepareMessageAttacksInHead(Daño, UserList(AtacanteIndex).Char.CharIndex))
        Call SendData(SendTarget.ToPCArea, VictimaIndex, PrepareMessageAttacksInHead(Daño, UserList(AtacanteIndex).Char.CharIndex))
...
'***************************************************
    ...
    With UserList(VictimIndex)
        If .flags.Meditando Then
            ...
            .Char.Particle = 0
            Call SendData(SendTarget.ToPCArea, VictimIndex, PrepareMessageCreateParticle(.Char.CharIndex, 0))
        End If
    End With
        ...
        If ObjInd > 0 Then
            If ObjData(ObjInd).Envenena = 1 Then
                ...
                If RandomNumber(1, 100) < 60 Then
                    ...
                    Call WriteCharVeneno(VictimaIndex, UserList(VictimaIndex).Char.CharIndex, UserList(VictimaIndex).flags.Envenenado)
                    Call WriteCharVeneno(AtacanteIndex, UserList(AtacanteIndex).Char.CharIndex, UserList(AtacanteIndex).flags.Envenenado)
