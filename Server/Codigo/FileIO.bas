...
'*************************************************
    ...
    With UserList(UserIndex)
        ...
        With .OrigChar
            ...
            .heading = CInt(UserFile.GetValue("INIT", "Heading"))
        End With
    ...
    On Error GoTo man
        ...
        For Map = 1 To NumMaps
            If val(GetVar(App.Path & MapPath & "Mapa" & Map & ".Dat", "Mapa" & Map, "BackUp")) <> 0 Then
                tFileName = App.Path & "\Resources\WorldBackUp\Mapa" & Map
...
On Error GoTo errh
    ...
    Get #FreeFileMap, , TempInt 'ZoneInt
    Get #FreeFileMap, , TempInt 'TerrainInt
    ...
    With MapInfo(Map)
        ...
        .LevelMIN = val(GetVar(MAPFl & ".dat", "Mapa" & Map, "ELVMIN", Asc("-")))
        .LevelMAX = val(GetVar(MAPFl & ".dat", "Mapa" & Map, "ELVMAX", Asc("-")))
        'If .LevelMAX < 0 Then .LevelMAX = STAT_MAXELV
...
'***************************************************
    ...
    Onirem.Map = GetVar(DatPath & "Ciudades.dat", "Onirem", "Mapa")
    Onirem.X = GetVar(DatPath & "Ciudades.dat", "Onirem", "X")
    Onirem.Y = GetVar(DatPath & "Ciudades.dat", "Onirem", "Y")
    
    Osurac.Map = GetVar(DatPath & "Ciudades.dat", "Osurac", "Mapa")
    Osurac.X = GetVar(DatPath & "Ciudades.dat", "Osurac", "X")
    Osurac.Y = GetVar(DatPath & "Ciudades.dat", "Osurac", "Y")
    
    Ciudades(eCiudad.cOnirem) = Onirem
    Ciudades(eCiudad.cOsurac) = Osurac
