...
'***************************************************
    ...
    Dim FaccionColor As Byte
    showName = val(GetVar(DatPath & "NPCs.dat", "NPC" & Npclist(NpcIndex).Numero, "ShowName"))
    FaccionColor = val(GetVar(DatPath & "NPCs.dat", "NPC" & Npclist(NpcIndex).Numero, "FaccionColor"))
    NPCName = IIf(showName > 0, Npclist(NpcIndex).name, vbNullString)
    ...
    If Not toMap Then
        Call WriteCharacterCreate(sndIndex, 4, Npclist(NpcIndex).Char.body, Npclist(NpcIndex).Char.Head, Npclist(NpcIndex).Char.heading, Npclist(NpcIndex).Char.CharIndex, X, Y, 0, 0, 0, 0, 0, NPCName, FaccionColor, 0)
...
If N < 30 Then
    ...
    Call WriteCharVeneno(UserIndex, UserList(UserIndex).Char.CharIndex, UserList(UserIndex).flags.Envenenado)
End If
