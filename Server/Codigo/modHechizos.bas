...
With UserList(UserIndex)
    ...
    If Hechizos(HechizoIndex).Envenena = 1 Then
        ...
        Call WriteCharVeneno(TargetIndex, .Char.CharIndex, .flags.Envenenado)
        Call InfoHechizo(UserIndex)
        HechizoCasteado = True
    End If
...
With UserList(TargetIndex)
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageAttacksInHead(Daño, UserList(UserIndex).Char.CharIndex))
        Call SendData(SendTarget.ToPCArea, TargetIndex, PrepareMessageAttacksInHead(Daño, UserList(UserIndex).Char.CharIndex))
