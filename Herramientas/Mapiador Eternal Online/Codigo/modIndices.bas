...
On Error GoTo ErrorHandler
    ...
    'Open files
    Handle = FreeFile()
    Open DirIndex & "\Graphics.ind" For Binary Access Read As Handle
    Seek #1, 1
...
ErrorHandler:
Close #1
    MsgBox "Error al intentar cargar el Graficos " & Grh & " de Graphics.ind en " & DirIndex & vbCrLf & "Err: " & Err.Number & " - " & Err.Description, vbCritical + vbOKOnly
...
On Error GoTo Fallo
    If FileExist(IniPath & "\ElementsIndices.ini", vbArchive) = False Then
        MsgBox "Falta el archivo '\ElementsIndices.ini'", vbCritical
        End
    End If
    Dim Leer As New clsIniReader
    Dim i As Integer
    Leer.Initialize IniPath & "\ElementsIndices.ini"
    ...
    Exit Sub
Fallo:
    MsgBox "Error al intentar cargar el indice " & i & " de \ElementsIndices.ini" & vbCrLf & "Err: " & Err.Number & " - " & Err.Description, vbCritical + vbOKOnly
End Sub
